...
We clean the text by converting all the letters from the cleaned responses to the lower case, and removing punctuation, numbers, empty words and extra white space.
...
We remove stopwords provided by the "tidytext" package and also add custom stopwords in context of the data.
...
stop_words <- stop_words %>%
  bind_rows(mutate(tibble(word), lexicon = "updated")) #add words in the stop_words and the level of lexicon is 'updated'
```
...
completed <- stemmed %>%
  ...
  bind_cols(dict) %>%
  anti_join(dplyr::row_number(), by = c("dictionary" = "word"))
```
...
```{r export data}
write_csv(hm_data, "../data/output/processed_moments.csv")
```
