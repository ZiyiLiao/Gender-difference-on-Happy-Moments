...
freq_f<- Corpus(VectorSource(female$text)) %>%
  DocumentTermMatrix() %>%
  as.matrix() %>% colSums() %>% data.frame()
freq_f <- order(freq_f$., decreasing = T) %>%
  data.frame(word = rownames(freq_f)[.], Freq = freq_f[.,])


freq_m <- Corpus(VectorSource(male$text)) %>%
  DocumentTermMatrix() %>%
  as.matrix() %>% colSums() %>% data.frame()
freq_m <- order(freq_m$., decreasing = T) %>%
  data.frame(word = rownames(freq_m)[.], Freq = freq_m[.,])
```
```{r warning=FALSE}
...
```{r barplot of POS}
ggplot(POS)+
  geom_bar(aes(x = POS$word, y = POS$nums), stat="identity", fill = 'darkseagreen') +
  facet_grid(~gender)+
...
wordcloud2(freq_f[1:100,2:3], color = colorVec_f, fontWeight = "bold",size=0.5, shape = 'cardioid', rotateRatio = 0.2)

colorVec_m = rep(c('blue', 'skyblue'), length.out=nrow(freq_m))
wordcloud2(freq_m[1:100,2:3], color = colorVec_m, fontWeight = "bold",size=0.5, shape = 'star', rotateRatio = 0.2)
```
